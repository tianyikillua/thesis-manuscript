...
As is already noted during the temporal discretization of the model in \cref{sec:timedis}, the damage problem \eqref{eq:crackmin} is not a genuine time evolution problem since neither the damage rate $\dot{\alpha}_t$ nor the strain rate $\dot{\eps}_t$ is involved. Velocity or dynamical effects that are illustrated in \cref{sec:kalthoff,sec:L-specimen} are solely due to inertia since the damage problem is rate-independent. In particular, the critical stress derived from the local damage criterion \eqref{eq:localdamagefirstorder} is a fixed value independently of the current loading rate that the material point is subject to.
