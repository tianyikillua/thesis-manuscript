...
\uhat_\tau'^2\leq 1.
\end{equation}
This implies along with the consistency condition \eqref{eq:damageconsis} that damage could first takes place wherever \eqref{eq:nuclecrit} is satisfied with an equality, see also \eqref{eq:damageinitiationsound}. Tension-compression asymmetry \eqref{eq:elasticTC} is not considered here. Using the jump condition, we have thus
\begin{itemize}
...
\subsection{Dynamic evolution of global quantities and fields}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{beam-sigm.png}
\caption{Fracture pattern with the parameters indicated in \cref{tab:beamconcrete}} \label{fig:beamsigm}
\end{figure}

This numerical result (\cref{fig:beamsigm}) is to be compared with the experimentally observed fracture pattern reported in \cite{Guilbaud:2015}. In the experiment, the structural failure is characterized by the nucleation of two main inclined cracks forming a cone at the center of the beam. Several minor vertical cracks are also observed under the cone as well as on the upper surface of the beam. However here a large diffuse damage region is produced at the center of the beam, which implies that the concrete material is totally disintegrated. Diffuse damage also takes place at the upper surface and near the lower supporting location. Although the central damage region is well delimited by a similar inclined cone, the presence of the diffuse damage compromises the use of gradient damage approaches for an appropriate modeling of brittle fracture.
\begin{figure}[htbp]
\centering
...
Similarly to \cref{sec:L}, we expect that the use of a more sophisticated damage constitutive law that introduces additional parameters could provide a better characterization of brittle behaviors of concrete for relatively small structures. The use of a smaller internal length is illustrated in the sequel. We artificially decrease the internal length to $\eta=\SI{10}{mm}$, which amounts to increase the critical stress to $\sigma_\mathrm{m}=\SI{7}{MPa}$. The simulation results are illustrated in \cref{fig:beamell0x1}.
\begin{figure}[htbp]
\centering
...
The formation of two main inclined cracks becomes more apparent. Some minor vertical cracks are also reproduced on the upper surface of the beam. However diffuse damage still takes place at the center of the beam. This implies that the current damage criterion (determined by the critical stress $\sigma_\mathrm{m}=\SI{7}{MPa}$) is satisfied almost everywhere at the center. It should be strengthened to prevent diffuse damage. In the sequel we consider a rate-dependent extension of the current damage criterion \eqref{eq:localdamagefirstorder} at the material level.

\subsection{With material rate-dependency}
As is already noted during the temporal discretization of the model in \cref{sec:timedis}, the damage problem \eqref{eq:crackmin} is not a genuine time evolution problem since neither the damage rate $\dot{\alpha}_t$ nor the strain rate $\dot{\eps}_t$ is involved. Velocity or dynamical effects that are illustrated in \cref{sec:kalthoff,sec:L} are solely due to inertia since the damage problem is rate-independent. In particular, the critical stress derived from the local damage criterion \eqref{eq:localdamagefirstorder} is a fixed value independently of the current loading rate that the material point is subject to.

A phenomenological rate-dependence of the damage criterion is used here to account for a higher resistance of concrete for large strain rates, see \cite{Hentz:2003}. The rate-dependence is characterized by a ``Dynamical Increase Factor'', which quantities the dynamical effects in the material resistance to damage as a function of the strain rate. A bilinear law proposed in \cite{Hentz:2003} is illustrated in \cref{fig:tdyn}. For low loading rates $\dot{\varepsilon}_t\leq\SI{1e-6}{s^{-1}}$, strain-rate effects are not apparent and the dynamical resistance agrees with the static one $\mathrm{DIF}=1$. For higher strain rates $\dot{\varepsilon}_t\approx \SI{100}{s^{-1}}$, the dynamic damage resistance is almost 10 times higher than the static one.
\begin{figure}[htbp]
\centering
\includegraphics[width=0.5\textwidth]{Tdyn.pdf}
\caption{Dynamical increase factor $\mathrm{DIF}$ proposed by \cite{Hentz:2003}} \label{fig:tdyn}
\end{figure}

This dynamical increase factor is now embedded into the local damage criterion \eqref{eq:localdamagefirstorder}. The energy release rate density with respect to damage $Y_t$ is modified as follows
\begin{equation} \label{eq:YtqtD}
Y_t^\mathrm{dyn}=-\frac{1}{\mathrm{DIF}}\frac{1}{2}\tens{A}'(\alpha_t)\eps(\vec{u}_t)\cdot\eps(\vec{u}_t)-w'(\alpha_t).
\end{equation}
According to \eqref{eq:YtqtD}, the critical stress becomes rate-dependent and is given by
\[
\sigma_\mc^\mathrm{dyn}=\mathrm{DIF}\cdot\sigma_\mc\,,
\]
where $\sigma_\mc$ is the classical critical stress \eqref{eq:sigc0}. Hence, the introduction of the strain-rate effect $\mathrm{DIF}$ increases the critical stress above which damage initiation takes place especially for higher loading rates.

In three dimensional calculations, an equivalent strain $\varepsilon_t$ is used to compute the dynamic increase factor $\mathrm{DIF}$. It is defined by
\[
\varepsilon_t=\sqrt{\frac{2}{3}\eps_t\cdot\eps_t}.
\]
Numerically a temporal low-pass filter is applied to reduce high-frequency oscillations. At time $t^n$, the equivalent strain rate is thus calculated by
\[
\dot{\varepsilon}^n=(1-\mathtt{F})\dot{\varepsilon}^{n-1}+\mathtt{F}\frac{\Delta \varepsilon_t}{\Delta t}\,,
\]
where $\mathtt{F}$ controls the filtering intensity. If $\mathtt{F}=1$, then no filter is applied. If $\mathtt{F}=0$, then the strain rate is fixed at its previous value. This parameter $\mathtt{F}$ can thus be used to control the strain-rate effect in the damage criterion.

The simulation results obtained with strain-rate effects $\mathtt{F}=\num{1e-4}$ are illustrated in \cref{fig:beamell0x11e-4}. The damage field represents better the experimentally observed fracture pattern: a central inclined cracked cone due to shear with some damage inside and some minor cracks on the upper surface. The use of a phenomenological dynamic increase factor leads to a better modeling of brittle fracture of concrete under severe impact.
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{beam-1e-4.pdf}
\caption{Fracture pattern obtained with $\eta=\SI{0.01}{m}$ and $\mathtt{F}=\num{1e-4}$} \label{fig:beamell0x11e-4}
\end{figure}

However the fracture behavior is extremely sensitive to the strain-rate effect through the $\mathtt{F}$-parameter. If this parameter is increases to $\mathtt{F}=\num{1e-3}$, the structural failure of the beam is characterized by tensile cracks that nucleate due to tension from the lower surface below the projectile, see \cref{fig:beamell0x11e-3}. Two inclined cracks due to shear also propagate from the lower supporting region.
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{beam-1e-3.png}
\caption{Fracture pattern obtained with $\eta=\SI{0.01}{m}$ and $\mathtt{F}=\num{1e-3}$} \label{fig:beamell0x11e-3}
\end{figure}

The damage distribution near the steel rebars is analyzed in \cref{fig:beam-alpha0}. It can be observed that due to the modeling hypothesis, no damage actually takes place in these rebars. Instead, the damage develops around the rebars, leading to partial debonding between the steel rebars and the concrete material.
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{beam-alpha0.pdf}
\caption{Damage distribution near the steel rebars} \label{fig:beam-alpha0}
\end{figure}

\subsection*{Conclusion}
In this experiment the dynamic gradient damage model is used to investigate the structural failure of a reinforced beam under severe impact. Similarly to \cref{sec:L}, it is illustrated that a more sophisticated damage constitutive law could be devised for the concrete material that introduces additional modeling parameters. Indeed, the internal length $\eta$ derived from the other material parameters in the \eqref{eq:at1} law may not be small by comparison with other structural length, which leads to a degenerated modeling of brittle fracture phenomena.

In the variational formulation of the dynamic gradient damage model outlined in \cref{def:dyngraddama}, material strain-rate effects are not taken into account. In this example a rate-dependent damage criterion \eqref{eq:YtqtD} is considered, hence the threshold for damage initiation increases with the strain-rate. A modeling parameter $0\leq\mathtt{F}\leq 1$ is used to control the intensity of such effects. This phenomenological model is applied to the present CEA impact test on beams. Satisfactory fracture pattern can be obtained by choosing an adequate value of the $\mathtt{F}$-parameter. However the dynamic fracture behavior seems to be extremely sensitive to this parameter.

By introducing such rate-dependency in the material constitutive modeling, the dynamic gradient damage model becomes no longer fully variational. Furthermore, the use of such dynamic increase factors in the damage criterion \eqref{eq:YtqtD} is purely phenomenological and recalls for a careful physical explanation and analysis. The energy balance condition \eqref{eq:dyngdeb} is no more valid since additional viscous dissipation takes place. Future work could be devoted to the investigation of such strain-rate dependent models.