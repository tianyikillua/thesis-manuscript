...
The damage profile at the initial crack tip $\vec{P}_0$ and at a crack tip after this critical time $t_\mc$ is compared with the optimal damage profile in \cref{fig:micro-macro}. In absence of micro-branching, the damage develops an optimal damage profile across the crack if the spatial discretization error (where $\alpha=1$ in an interval of width $2h$) is ignored, see \cref{sec:ellipticregul}. However during micro-branching, the damage band is even more widened. This implies further damage dissipation and may be considered to represent macroscopically the interaction of micro-branches. 
\begin{figure}[htbp]
\centering
...
Newt we propose to perform a zoom in time and in space at $t_\mc$ to better understand the physics behind the micro-branching event.
