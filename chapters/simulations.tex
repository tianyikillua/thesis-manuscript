...
\[
(\gc)_\mathrm{eff}=\left(1+\frac{3h}{8\ell}\right)\gc.
\]
...
\paragraph{Problem setting} The problem setting is adapted from \cite{HakimKarma:2005,HakimKarma:2009}. At time $t=0$ a hard device $\vec{U}$ is prescribed on the boundary $\partial\Omega$ of a unit ball centered at the origin $\Omega=B_1(\vec{0})$, see \cref{fig:kinking}.
\begin{figure}[htbp]
\centering
...
\end{figure}
An existing \emph{stationary} plane crack $\Gamma_t=[-1,0]\times\set{0}$ modeled by an initial damage field is subject to the mixed-mode loading condition $\vec{U}$ and the objective is to compare the apparent kinking angle $\theta$ with some theoretic criteria commonly used in fracture mechanics to supplement Griffith's law, cf. \cref{sec:theoexpcrit}. The imposed displacement $\vec{U}$ corresponds to the asymptotic expansion of the displacement field near the crack tip $\vec{P}_0=(0,0)$ with the opening and sliding intensity factors $K_1>0$ and $K_2\geq 0$, see \eqref{eq:singularform}. The plane stress condition is assumed. By virtue of Irwin's formula \eqref{eq:GasafunctionofK} the imposed energy release rate is given by
\[
...
In this section the first-order quasi-static gradient damage model (\cref{def:firstorderqs}) is used. The quasi-static condition constitutes a natural setting to prescribe a given deformation state near the crack tip. According to the convergence study in \cref{sec:antiplane}, the quasi-static gradient damage model can indeed be considered as a limit model of the dynamic one in absence of fast or unstable crack propagation. However according to the analysis of \cite{ChambolleFrancfortMarigo:2009}, the temporal crack propagation is necessarily \emph{unstable} in presence of a kink. Future work will be devoted to a full dynamic investigation of this problem.

A non-structured triangular mesh is used to discretize the domain $\Omega$. The material, loading and numerical parameters used in this problem are summarized in \cref{tab:kinking}. The damage constitutive law \eqref{eq:at1} is used.
\begin{table}[htbp]
\centering
\caption{Material, loading and numerical parameters for the plane crack kinking experiment.} \label{tab:kinking}
\begin{tabular}{cccccc} \toprule
$E$ & $\nu$ & $\gc$ & $\ell$ & $\overline{G}$ & $h$ \\ \midrule
1 & 0.2 & 1 & 5\% or 1\% & 1.1 or 1.5 & $\ell/5$ \\ \bottomrule
\end{tabular}
\end{table}

\paragraph{Simulation results and discussion} Several simulations corresponding to different $K_2/K_1$ ratio are performed. Typical numerical results are illustrated by the damage field after kinking in \cref{fig:kinking_k2k1}.
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{kinking_G1x5.pdf}
\caption{Damage field obtained with $\ell=1\%$ and $\overline{G}=1.5$ for three $K_2/K_1$ ratios: (a) $K_2/K_1=0.6$, (b) $K_2/K_1=0.8$ and (c) $K_2/K_1=1$} \label{fig:kinking_k2k1}
\end{figure}
The apparent kinking angle predicted by the gradient damage model is compared in \cref{fig:kinking_comp} with some commonly used kinking criteria in fracture mechanics, see \cref{sec:theoexpcrit}.
\begin{figure}[htbp]
\centering
...
\subsection{Physical insights into the branching mechanism}
