...
\subsection{Modeling of tension-compression asymmetry}
...
In this section the scope and objectives of the present work are defined. The work is divided into several thematic subjects and is both theoretical and numerical in nature:
\begin{itemize}
\item \textbf{Going dynamical} concerns first the formulation and numerical implementation of gradient damage models in a dynamic setting. Dynamic extension of previous analyses or results in the quasi-static setting also falls into this category.

\item \textbf{Bridging the link with phase field approaches} ($\alpha\leftrightarrow\phi$) concerns a comparison between the gradient damage model and other phase-field approaches. This implies a theoretic and numerical study of different parameters in the model.

\item \textbf{Better understanding of gradient damage modeling of fracture} ($\nabla\alpha\leftrightarrow\Gamma$) aims to highlight specific properties of gradient damage approaches when regarded as a genuine physical model of fracture.

\item \textbf{Experimental validation} compares the gradient damage model with experimental facts.
\end{itemize}
This classification is summarized in \cref{tab:novelty} and will be used at the beginning of each section from the next chapter to situate the content that follows. 
\begin{table}[htbp]
\centering
\caption{Thematic subjects covered by the present research on dynamic gradient damage models tackled by theoretical or numerical approaches. This table will be used at the beginning of each section from the next chapter to situate the content that follows. For that, a or several \rightthumbsup\ symbol(s) will be placed in the corresponding line and column.} \label{tab:novelty}
\begin{tabular}{ccccc} \toprule
& Going dynamical & $\alpha\leftrightarrow\phi$ & $\nabla\alpha\leftrightarrow\Gamma$ & Experimental validation \\ \midrule
...
The first objective of the present work is to propose a formulation of gradient damage models in a dynamic setting. Due to the link between the Variational Approach to Fracture and the gradient damage model indicated in \cref{sec:fm98,sec:graddamage}, this will be achieved by embedding the dynamic extension of the $\ell$-regularized fracture model proposed in \cite{Bourdin:2011} into the variational framework of \cite{PhamMarigo:2010-1} in terms of several physical properties.

Numerical implementation of the model constitutes the next main objective of this work. Both explicit and implicit dynamics settings are considered even though the efforts are concentrated on the former one. The efficiency of the numerical implementation will be illustrated by several simulations results.

\subsection{Bridging the link with phase field approaches}
Phase-field modeling of fracture is gaining popularity over the last few years within the computational mechanics community pioneered by the work of \cite{MieheHofackerWelschinger:2010,MieheWelschingerHofacker:2010}. In parallel, the phase-field model originated from the physics community constitutes another continuum mechanics approach for studying fracture problems, see for example \cite{KarmaKesslerLevine:2001,HakimKarma:2009}. Following the classification given in \cref{sec:discretemodels}, they both also adopt a smeared description of cracks. They will be compared in \cref{sec:phasefields}

\cite{BordenVerhooselScottHughesLandis:2012,HofackerMiehe:2012,SchlueterWillenbuecherKuhnMueller:2014}.

%Highlight the differences with respect to existing phase-field methods w(alpha)=alpha^2

We believe this constitutes a difficulty when using the gradient damage model as a phase-field approach to fracture. TC

\subsection{Better understanding of gradient damage modeling of fracture}
...
Some representative simulation examples will be then given to highlight some of its characteristics. 

We then investigate the quasi-static limit of our dynamic model when the loading speed tends to zero

give some industrial examples to roughly verify and validate this dynamic model
