...
Based on the kinematic description of cracks in the continuum body, different fracture mechanics models can be classified into three categories:
\begin{enumerate}
\item \textbf{Discrete modeling approach}\index{Fracture!Discrete modeling approach} where the crack is considered as an explicit interface in the body across which the displacement vector is discontinuous. It includes but is not limited to the classical Griffith's theory of fracture mechanics \cite{Freund:1990}, the Variational Approach to Fracture \cite{FrancfortMarigo:1998,BourdinFrancfortMarigo:2008} as well as some possible extensions to the dynamic setting considered by \cite{Larsen:2010}

\item \textbf{Smeared modeling approach}\index{Fracture!Smeared modeling approach} where strong discontinuities are regularized by strain localizations within a finite and thin band

\item A combination of the previous two approaches where a \textbf{transition}\index{Fracture!Transition between discrete-smeared descriptions}} between a smeared description and a discrete description of cracks can be achieved
\end{enumerate}

In the first approach, the crack surface as well as its current crack tips are  tracked algorithmically 
