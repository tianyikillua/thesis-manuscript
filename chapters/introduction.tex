...
The current formulation of gradient damage models in the rate-independent evolution framework in the sense of \cite{Mielke:2005} is achieved in \cite{PhamMarigo:2010,PhamMarigo:2010-1}. The scalar field $\alpha$ introduced in the elliptic regularization (see \cref{sec:ellipticregul}) of the Variational Approach to Fracture is now interpreted as a damage variable ranging from 0 (intact material) to 1 (totally damaged material). The elastic-damage evolution is variational in nature and governed by several physics-motivated principles based on the definition of a potential energy of the body $\Omega$. Adopting the notation used in a review paper \cite{PhamAmorMarigoMaurini:2011} of gradient damage models, the potential energy $\mathcal{P}$ reads
\begin{equation} \label{eq:Pgd}
\mathcal{P}(\vec{u},\alpha)=\int_\Omega\left( a(\alpha)\psi\bigl(\eps(\vec{u})\bigr)+w(\alpha)+\frac{1}{2}w_1\ell^2\nabla\alpha\cdot\nabla\alpha\right)\dx
\end{equation}
where $\alpha\mapsto a(\alpha)$ and $\alpha\mapsto w(\alpha)$ denote two damage constitutive functions and $w_1=w(1)$. Two interdependent interpretations can be given to motivate such construction of the gradient damage models.
\begin{itemize}
\item As the name indicates, gradient damage models belongs to the general theory of continuum damage mechanics\index{Damage mechanics} initiated by the pioneer work of \cite{Kachanov:1958} and then formalized in the thermodynamic framework through the work of \cite{LemaitreChaboche:1978} and others. Compared to these conventional damage models, the gradient damage approach consists of introducing an additional damage gradient which \emph{regularizes} the original strain-softening mechanism.

\item One can not fail to realize the similarity between the gradient damage potential energy \eqref{eq:Pgd} and that of the regularized fracture model \eqref{eq:Pualpha} initially proposed in \cite{BourdinFrancfortMarigo:2000}. Written in the form of \eqref{eq:Pgd}, the damage gradient potential energy captures the Ambrosio--Tortorelli regularization\index{Ambrosio--Tortorelli regularization} via a particular choice of the constitutive functions. Based on rigorous $\Gamma$-convergence results established in \cite{Braides:2002}, \eqref{eq:Pgd} can also be considered as an approximation of the sharp-interface model, as long as these constitutive functions verify certain properties.
\end{itemize}
These two interpretations are schematized in \cref{fig:twointerpretationdamage}.
\begin{figure}[htbp]
\centering
\[
\begin{CD}
\mathcal{P}_0(\vec{u},\alpha) @>\text{+$\nabla\alpha$}>> \mathcal{P}_\ell(\vec{u},\alpha) \\
@. @AA\text{$\ell$-regul.}A \\
@. \mathcal{P}(\Gamma)
\end{CD}
\]
\caption{The gradient damage model represented by the potential energy $\mathcal{P}_\ell$ admits two different interpretations: on the one hand, it can be considered as a regularization of local damage models $\mathcal{P}_0$; on the other hand, it admits a rigorous link with the sharp-interface Francfort-Marigo variational model $\mathcal{P}$ defined in \eqref{eq:fm98}} \label{fig:twointerpretationdamage}
\end{figure}

\paragraph{Regularization of conventional damage models} From a damage mechanics viewpoint, gradient damage models \eqref{eq:Pgd} can be considered as a regularization of conventional mathematically and mechanically ill-posed local strain-softening models. Either in the quasi-static setting \cite{Benallal:1993} or in the dynamic setting \cite{Bazant:1985aa}, local models could produce extreme strain localization within a vanishing band. Hence material failure occurs without any energy dissipation. This implies a spurious mesh dependence of the finite-element results, since the mesh size determines the localization band and the energy dissipated in producing such strain localization. An additional characteristic length must be introduced through the use of some non-local operators in the material constitutive behavior or in the damage criterion, see \cite{LorentzAndrieux:2003} for a review of several approaches.

\cite{PeerlingsBorstBrekelmansVree:1996} strain gradient in the criterion
\cite{Pijaudier-CabotBazant:1987,Comi:2001} integral of the driving force in the damage criterion
\cite{BazantPijaudier-Cabot:1988} integral of the damage variable
\cite{LorentzAndrieux:1999} damage variable


 Why damage gradient ? cite \cite{LeMauriniMarigoVidoli:2015} ; variational approach to damage (Illuyshin + Asymptotic development)

\paragraph{Improvement of the Variational Approach to Fracture} (global -> local, ell no longer nulerical to have sigc with griffith)
