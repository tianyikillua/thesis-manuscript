...
\]
which agrees exactly with that used in \cite{MieheHofackerWelschinger:2010,MieheWelschingerHofacker:2010,HofackerMiehe:2012,HofackerMiehe:2013}. In \cite{BordenVerhooselScottHughesLandis:2012,SchlueterWillenbuecherKuhnMueller:2014}, a non-essential rescaling of the internal length $\eta=2\widetilde{\eta}$ is performed. As stated in \cref{sec:ingredients}, this constitutive law originates from the Ambrosio and Tortorelli regularization model introduced in \cite{BourdinFrancfortMarigo:2000} and enjoys hence the desired $\Gamma$-convergence property. In this respect, these phase-field models correspond to a special case of gradient damage models.
...
\end{equation}
where derivatives of the decomposed strains $\eps^\pm$ with respect to the total strain appear. Fortunately, as $\partial_{\eps}\eps^+\in\mathcal{C}$ and $\partial_{\eps}\tens{A}\eps^-\in\mathcal{C}^*$, we have due to \eqref{eq:vipos}
...
For the masonry-like model \cite{FreddiRoyer-Carfagni:2010}, an explicit formula of the positive strain $\eps^+$ as a function of the total strain $\eps$ is obtained for both two-dimensional or three-dimensional problems in the isotropic case. The solution is derived from the minimization structure \eqref{eq:variationalepspos} or equivalently the variational inequality \eqref{eq:vipos}. The coaxiality between $\eps^+$ and $\eps$ is also exploited due to isotropy, see \cite{Del:1989,AlfanoRosatiValoroso:2000}. In the three dimensional case, after the spectral decomposition the total strain tensor reads $\eps=(\varepsilon_1,\varepsilon_2,\varepsilon_3)$ in its principal basis where without generality we assume $\varepsilon_1\geq\varepsilon_2\geq\varepsilon_3$. According to \cite{Sacco:1990}, $\eps^+$ in the same principal basis is then given by
\begin{enumerate}
\item If $\varepsilon_3\geq 0$, then $\eps^+=\eps$
\item Else If $\varepsilon_3<0$ and $\varepsilon_2+\nu\varepsilon_3\geq 0$, then $\eps^+=(\varepsilon_1+\nu\varepsilon_3,\varepsilon_2+\nu\varepsilon_3,0)$
\item If $\varepsilon_1<0$, $\nu\varepsilon_1+\varepsilon_2<0$ et $\nu(\varepsilon_1+\varepsilon_2)+(1-\nu)\varepsilon_3\geq 0$, alors
\[
\eps^+=(0, 0, \frac{\nu(\varepsilon_1+\varepsilon_2)}{1-\nu}+\varepsilon_3)
\]
\item Si $\varepsilon_1<0$, $\nu\varepsilon_1+\varepsilon_2<0$ et $\nu(\varepsilon_1+\varepsilon_2)+(1-\nu)\varepsilon_3<0$, alors $\eps^+=\vec{0}$
\end{enumerate}

\begin{figure}[htbp]
\includegraphics[width=0.65\textwidth]{epspos2d.pdf}
\caption{\scriptsize Décomposition $\eps=\eps^++\eps^-$ en déformation plane}
\end{figure}
